...
def findAttractor(x,D,h,minRange):
    ...
    # while(np.linalg.norm(x_t_next - x_t,ord=2,axis=0) > minRange):
        ...
        t += 1
    print("迭代了：",t)
    #将迭代过程中漂移的距离颠倒，即列表中的第一个距离为最后一次迭代的漂移距离
    dis_list.reverse()
    #计算最后三步迭代的距离累加和，用于后续寻找局部最大密度可达
    return_count = 0
    for Num in range(3):
...
if __name__=="__main__":
    ...
    #     print(D[i])
    #调用DENCLUE方法进行聚类
    eachCulsterAttractors,clusterResultSet,clusterResultLabel = DENCLUE(D,0.35,0.1625,0.0001,Label)     #0.35  0.1625   #0.34  0.1718

    #统计并输出聚类后的类簇个数，以及各簇中的数据个数
